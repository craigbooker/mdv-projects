...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    TweetDetailView *myDetailView = [[TweetDetailView alloc] initWithNibName:@"TweetDetailView" bundle:nil];
    if(myDetailView != nil)
    {
        [self.navigationController pushViewController:myDetailView animated:YES];
        Tweet *showTweetDetails = [mainArray objectAtIndex:indexPath.row];
        [myDetailView:showTweetDetails:content title:showTweetDetails.dateCreated];
    }
